...
-- Drivers: Can insert history records for their own credentials
...
SELECT 'Migrations 007-015 applied successfully!' as status;
...
SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'credential_submission_history') as credential_submission_history_exists;

-- Verify triggers exist
SELECT tgname FROM pg_trigger WHERE tgname IN ('on_auth_user_created', 'trigger_update_has_payment_info', 'trigger_update_has_availability', 'driver_credentials_history_trigger', 'vehicle_credentials_history_trigger');

-- Verify functions exist
SELECT proname FROM pg_proc WHERE proname IN ('get_driver_required_credentials', 'get_vehicle_required_credentials', 'calculate_credential_expiration');

-- Verify key policies
SELECT policyname FROM pg_policies WHERE tablename IN ('driver_onboarding_progress', 'driver_availability', 'driver_payment_info', 'credential_submission_history') ORDER BY tablename, policyname;
