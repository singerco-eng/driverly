...
/**
 * Admin driver credential detail page with review capabilities
 */
export default function DriverCredentialDetail() {
  const { driverId, credentialId } = useParams<{ driverId: string; credentialId: string }>();
  const navigate = useNavigate();
  const { toast } = useToast();
  const queryClient = useQueryClient();

  const backPath = `/admin/drivers/${driverId}`;

  // Fetch credential with type and driver info
  const { data: credential, isLoading } = useQuery({
    queryKey: ['driver-credential', credentialId],
    queryFn: async () => {
      ...
      return data as (DriverCredential & {
        ...
        driver: { id: string; user: { id: string; full_name: string; email: string } };
      }) | null;
    },
    ...
    return (
      <div className="space-y-6">
        <Card className="p-12 text-center">
          ...
          <Button onClick={() => navigate(backPath)}>Back to Driver</Button>
        </Card>
      </div>
    );
  }
