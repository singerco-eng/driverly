-- =============================================================================
-- Migration: 024_admin_ensure_driver_credential.sql
-- Description: Allow admins to create driver credentials for any driver in their company
-- =============================================================================

-- Function for admins to ensure a driver credential exists for any driver
CREATE OR REPLACE FUNCTION admin_ensure_driver_credential(
  p_driver_id UUID,
...
BEGIN
  ...
  IF v_user_role NOT IN ('admin', 'super_admin', 'coordinator') THEN
    RAISE EXCEPTION 'Only admins can create credentials for other drivers';
  END IF;

  -- Ensure admin is in the same company (unless super_admin)
  IF v_user_role != 'super_admin' AND v_user_company_id != p_company_id THEN
    RAISE EXCEPTION 'Admin can only manage drivers in their own company';
  END IF;
...
GRANT EXECUTE ON FUNCTION admin_ensure_driver_credential(UUID, UUID, UUID) TO authenticated;
